...
export class SummaryComponent implements OnInit {
  ...
  transactions: any[] = [];
  transactionsByType: object[] = [];

  constructor() { }

  ngOnInit() {
    this.transactions = transactions;
    this.getTransactionsByType(this.getTypes());
  }

  getTypes(): any[] {
    return [...new Set(this.transactions.map((transaction: any) => transaction.type))];
  }

  getTransactionsByType(types: string[]): void {
    for (const type of types) {
      switch (type) {
        case 'income':
            this.transactionsByType[0] = {name: type, transactions: this.filterTransactionsByType(type)};
          break;
        case 'outcome':
            this.transactionsByType[1] = {name: type, transactions: this.filterTransactionsByType(type)};
          break;
        case 'loan':
            this.transactionsByType[2] = {name: type + 's', transactions: this.filterTransactionsByType(type)};
          break;
        case 'investment':
            this.transactionsByType[3] = {name: type + 's', transactions: this.filterTransactionsByType(type)};
          break;
        default:
          break;
      }
  ...
  filterTransactionsByType(type: string): object[] {
    return this.transactions.filter((transaction: any) => transaction.type === type);
  }

  calculateAmount(transactions: any[]): number {
    return transactions.reduce((accumulator: number, transaction: any) => accumulator + transaction.amount, 0);
  }
